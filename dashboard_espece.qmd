...
histo_line(df_histo = df_sp %>% 
             ...
             group_by(date) %>%
             summarise(sum_ab = n()),
           df_ligne = df_ab_tot, div = 1, ytxtsec = "Observations totale", ytxt = "Observations")
```
...
histo_line(df_histo = df_sp %>% 
             ...
             group_by(date) %>%
             summarise(sum_ab = n()),
           df_ligne = df_ab_tot, div = div_obs, ytxtsec = "Observations totale", ytxt = "Observations")
```
...
a = sort(c(unique(df_bary_all_sp$nom_espece), "Jardins"))
b = rep("#5F5F5F", length(a))
b[which(a == "Jardins")] = "#0baaff"
b[which(a == sp_name)] = "#d50404"

gg = ggplot(france) +
  geom_sf(fill = "#f0f0f0", color = "#a0a0a0") +
  geom_point(data = rbind(df_bary_all_sp, df_bary_base),
             aes(x = longitude, y = latitude, color = nom_espece, frame = annee)) +
  scale_color_manual(values = b) +
  theme_minimal()
...
#   cat(sprintf(paste0("\n\n##", year, "\n\n")))
#   x <- gg_carte(an = year, df_sp = df_sp, france = france)
#   cat(htmltools::renderTags(as.widget(x))$html)
#   # l[[i]] <- gg_carte(an = year, df_sp = df_sp, france = france)
#   # i = i+1
# }
# # l
```
...
```{r}
heatmap(df_heatmap, Colv = NA, Rowv = NA)
```
