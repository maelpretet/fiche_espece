...
df_sp %>%
  ...
  ggplot() +
   ...
   geom_bar(aes(x = date, y = sum_ab), color = "#8A173A", fill="#ab0739", stat="identity") +
   geom_line(data = df_nb_obs, aes(x = date, y = n*div_obs, group = 1), color = "#ff795c") +
   theme_cowplot() +
   ...
   ylab("Abondance") +
   scale_y_continuous(sec.axis = sec_axis(~./div_obs, name = "Nombre d'observations"))
```
...
div_obs = max((df_sp %>%
                 mutate(date_mois = strftime(as.Date(date_collection),
                                             "%G-%m")) %>%
                 group_by(date_mois) %>%
...
df_sp %>%
  ...
  ggplot() +
   # geom_line(aes(x = date, y = sum_ab), color="#8A173A") +
   geom_bar(aes(x = date_mois, y = sum_ab), color = "#8A173A", fill="#ab0739", stat="identity") +
   geom_line(data = df_nb_obs, aes(x = date_mois, y = n*div_obs, group = 1), color = "#ff795c") +
   theme_cowplot() +
   scale_x_date(date_labels = "%Y-%b", date_breaks = "9 months") +
   theme(axis.text.x = element_text(angle = 0, size = 8),
         axis.title.y.left =  element_text(size = 12, color = "#ab0739"),
...
for(i in 1:3) {
  ...
  print(df_histo %>%
    ggplot() +
      ...
      xlab(lst_param[[i]][[3]]) +
      ylab("Abondance moyenne\n(sur les jardins avec espèce observée)") +
      theme_cowplot())
...
for(an in 2019:2024) {
  ...
  print(carte_ab(shape_map = france, fill_map = df_dep_an$cl_ab,
                 fill_color = couleurs, fill_cat = cat,
...
`{r} nb_max_ab$sum_ab`
individus en `{r} nb_max_ab$annee` en `{r} nb_max_ab$nom_departement` (`{r} nb_max_ab$nom_region`)
:::
...
carte_ab(shape_map = france, fill_map = df_dep$cl_ab, fill_color = couleurs,
         fill_cat = cat, fill_title = "Abondance entre 2019 et 2024")
```
...
#| fig-width: 6
#| fig-height: 4
esp_coocc = c(sp_name, df_oui$nom[1:5])

df_coocc = df_all_sp %>%
  filter(nom_espece %in% esp_coocc)
...
aes_echarts(plot_e = plot_e1, xlab = "Semaine de collection",
            ylab = "Indice",
            title = "Indice par semaine selon les années",
