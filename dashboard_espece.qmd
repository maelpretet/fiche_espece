...
#| fig-width: 6
#| fig-height: 4
for(i in sort(unique(df_sp$annee))) {
  cat(sprintf(paste0("\n\n##### ", i,"\n\n")))
  print(i)
  # a = plot(x = c(1,2,3), y = c(10,20,30))
  # print(a)
  df_mois = data.frame(mois = c("01", "02", "03", "04", "05", "06",
                              "07", "08", "09", "10", "11", "12"),
                     jardin_id = 0,
  ...
  if (i == as.numeric(strftime(Sys.Date(), "%Y")) ){
...
for (year in sort(unique(df_sp$annee))) {
  ...
  img_animated <- image_animate(img_joined, fps = 2) ## Animation (2 frames par seconde)
...
Abondance relative : observations d'une espèce par semaine / observations totale de l'espèce depuis 2019
