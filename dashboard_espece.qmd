...
a = df_all_sp %>%
  group_by(jardin_id, participation_id) %>%
  summarise(n = n())
...
df_sp %>%
    ...
    e_x_axis(name="Semaine de collection",
...
# df_migration <- st_as_sf(df_migration, coords = c("longitude", "latitude"),
#                          crs = "WGS84", remove = FALSE)

# ggplot(france) +
#   geom_sf() +
#   geom_sf(data = df_migration) 
